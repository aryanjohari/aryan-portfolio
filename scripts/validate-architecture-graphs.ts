...
console.log(
  "project              | nodes | tour | caps | layout | warnings",
);
console.log(
  "---------------------|-------|------|------|--------|----------",
);
...
for (const [slug, raw] of Object.entries(RAW)) {
  ...
  if (!result.ok) {
    failed = true;
    console.error(`${slug.padEnd(20)} | INVALID`);
  ...
  const capCount = result.graph.captions?.length ?? 0;
  const warnings = [...result.warnings];
  if (capCount < summary.tourStops) {
    warnings.push({
      path: "captions",
      message: `missing captions for ${summary.tourStops - capCount} tour stop(s)`,
    });
  }
  const warn =
    warnings.length > 0 ? warnings.map((w) => w.message).join("; ") : "—";
  console.log(
    `${slug.padEnd(20)} | ${String(summary.nodeCount).padStart(5)} | ${String(summary.tourStops).padStart(4)} | ${String(capCount).padStart(4)} | ${summary.layoutMode.padEnd(6)} | ${warn}`,
  );
}
